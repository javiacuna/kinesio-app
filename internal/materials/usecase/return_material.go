...
func (uc *ReturnMaterialUseCase) Execute(ctx context.Context, loanID uuid.UUID, actorEmail string, actorRole string) (domain.MaterialLoan, error) {
	...
	if err := uc.repo.MarkReturned(ctx, loanID, now, trimOptionalString(actorEmail), trimOptionalString(actorRole)); err != nil {
		return domain.MaterialLoan{}, err
	}
